...
per_page = 2000
progress_every = 25
whitelist = Path("cpe_whitelist.csv")  # Save to working directory
...
if mode == "Search & Save CPEs":
    ...
    if all_results:
        if st.button(f"Save {len(all_results)} result(s) to file"):
            ...
            saved_df = pd.DataFrame(all_results)
            st.download_button("Download Results as CSV", saved_df.to_csv(index=False), "saved_cpes.csv")

elif mode == "Run CVE Query" and api_key:
    ...
    if uploaded_file is not None:
        ...
        st.warning("Please upload a valid CPE whitelist CSV file.")
